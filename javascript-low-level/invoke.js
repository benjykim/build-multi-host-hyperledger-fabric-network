...
var orderertlscert_path = path.resolve(network_path, 'crypto-config', 'ordererOrganizations', 'example.com', 'tlsca', 'tlsca.example.com-cert.pem');
var orderertlscacert = fs.readFileSync(orderertlscert_path, 'utf8');
...
async function invoke() {
	...
	try {
		...
		// -- orderer instance to represent a orderer on the channel
		...
		const peer2 = fabric_client.newPeer('grpcs://10.21.4.11:7051', {
			'ssl-target-name-override': 'peer0.org2.example.com',
			pem: org2tlscacert
		});

		const peer3 = fabric_client.newPeer('grpcs://10.21.1.22:7051', {
		...
		await channel.initialize({ discover: false, asLocalhost: false, target: peer });
		...
		// check the results to decide if we should send the endorsement to be orderered
